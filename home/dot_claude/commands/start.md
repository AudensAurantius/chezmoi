...
7. **Offer to scaffold a task directory.** Check whether any `tasks/*/` directory has a `.beads` file whose contents equal `<resolved_id>` (run `grep -rl "^<resolved_id>$" tasks/ --include=".beads" 2>/dev/null`). If found, skip this step silently. Otherwise, ask the user on one line: `Scaffold tasks/<id>/? Enter a descriptive slug (e.g. "fix-auth-cookie") or N to skip:`. Do not try to judge appropriateness from the bead description — the user decides. On a non-empty slug (anything other than `n`/`N`/empty), invoke `/task-init <slug> --bead <resolved_id>` yourself; on `n`/`N`/empty, skip and continue. The task directory will be named `<bead-id>[-<jira-key>]-<slug>` per the J121 convention.
